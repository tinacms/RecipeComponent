"use strict";Object.defineProperty(exports, "__esModule", {value: true});

var _XCCLPY67cjs = require('./XCCLPY67.cjs');


var _5X5VM34Kcjs = require('./5X5VM34K.cjs');




var _MFAH5VLWcjs = require('./MFAH5VLW.cjs');



var _7EQBAZ46cjs = require('./7EQBAZ46.cjs');

// src/popover/popover-dismiss.tsx
var TagName = "button";
var usePopoverDismiss = _MFAH5VLWcjs.createHook.call(void 0, 
  function usePopoverDismiss2(_a) {
    var _b = _a, { store } = _b, props = _7EQBAZ46cjs.__objRest.call(void 0, _b, ["store"]);
    const context = _5X5VM34Kcjs.usePopoverScopedContext.call(void 0, );
    store = store || context;
    props = _XCCLPY67cjs.useDialogDismiss.call(void 0, _7EQBAZ46cjs.__spreadValues.call(void 0, { store }, props));
    return props;
  }
);
var PopoverDismiss = _MFAH5VLWcjs.forwardRef.call(void 0, function PopoverDismiss2(props) {
  const htmlProps = usePopoverDismiss(props);
  return _MFAH5VLWcjs.createElement.call(void 0, TagName, htmlProps);
});




exports.usePopoverDismiss = usePopoverDismiss; exports.PopoverDismiss = PopoverDismiss;
